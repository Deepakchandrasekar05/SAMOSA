...
st.set_page_config(page_title="S.A.M.O.S.A", page_icon="./logo.png", layout="wide")
...
with st.container():
    ...
    with right_column:
        ...
        style = '''<style>h1 {padding: 2000px;font-size: 20px;padding-left:1px;}
        h2 {padding: 120px;font-size: 40px;padding-left:1px;}</style>'''
...
def plot(df):
    ...
    # Using a unique identifier for the chart option key
    if 'chart_selection_counter' not in st.session_state:
        st.session_state['chart_selection_counter'] = 0

    chart_selection_counter = st.session_state['chart_selection_counter']
    unique_key = f"chart_option_{chart_selection_counter}"
    st.session_state['chart_selection_counter'] += 1
